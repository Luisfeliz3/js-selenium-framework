...
const And = Then;
const LoginPage = require('../pages/dashboard');
...
And('I should see a trsx button', async function () {
  const loginPage = new LoginPage(this.driver);
  await this.driver.wait(async () => await loginPage.getTrsxButton(), timeouts.STEP_TIMEOUTS.TIMEOUT);
});

When('I should see payment logo', async function () {
  const loginPage = new LoginPage(this.driver);
  await this.driver.wait(async ()=> await loginPage.highLightElement()) 
  await this.driver.wait(async () => await loginPage.getLogo(), timeouts.STEP_TIMEOUTS.TIMEOUT);
});
